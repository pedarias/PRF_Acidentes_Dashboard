"""
Script para carregar dados do CSV para o SQLite
"""
import os
import pandas as pd
import numpy as np
...
def process_csv(file_path):
    ...
    try:
        ...
        if len(df) > 10000:
            ...
            logger.info(f"Usando amostra de 10000 registros para evitar problemas de memória")
            return df_sample
...
def load_to_sqlite(df, batch_size=1000):
...
def gerar_trechos_perigosos(ano=None):
...
def main():
    ...
    try:
        ...
        if ano:
            gerar_trechos_perigosos(int(ano))
        else:
            gerar_trechos_perigosos()
